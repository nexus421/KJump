package bayern.kickner.totp

import bayern.kickner.totp.Totp.generateSecret
import bayern.kickner.totp.Totp.verifyCode
 ...
 * ### Usage Flow:
 * 1. **Registration**:
 *    - The **Server** calls [generateSecret] to create a new shared secret.
 ...
 *
 * 2. **Verification**:
 *    - The **User's App** generates a 6-digit code based on the shared secret and the current time.
 *    - The **User** provides this code to the **Server**.
 *    - The **Server** retrieves the stored secret and calls [verifyCode] with the user-provided code.
 *
...
object Totp {
    ...
    fun verifyCode(
        ...
        for (i in -window..window) {
            if (generateCodeAtInterval(secret, currentInterval + i) == code) {
                return true
            }
        }
        return false
